...
import { LayoutPass, px, em, make_request, available, exact, render_svg, evaluate } from 'gum-next-core'
import type { Fragment, MathStyle, Element, FontProvider } from 'gum-next-core'
...
describe('fractions, roots, and delimiters', () => {
  ...
  test('compound elements retain sizing, immutable sharing, and prepared work across offers', () => {
    ...
    expect(tiny.overflow.right).toBeGreaterThan(0)
    expect(calls).toBe(count)
